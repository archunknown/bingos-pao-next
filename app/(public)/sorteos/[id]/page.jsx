...
export default async function SorteoPublicoPage({ params }) {
  ...
  const [{ data: sorteo }, { data: configRows }] = await Promise.all([
    supabase
      .from('sorteos')
      .select('id, nombre, tipo, precio_participacion, fecha_sorteo, descripcion, premios(id, nombre, monto, descripcion_premio, cantidad, orden, visible)')
      .eq('id', id)
      .eq('estado', 'activo')
      .single(),
  ...
  // Generate QR public URLs
  for (const key of ['qr_yape', 'qr_plin']) {
